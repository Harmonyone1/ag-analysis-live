...
print('='*70)
print('TSLA - COMPREHENSIVE ANALYSIS')
print('='*70)
print()

# Check if TSLA exists
if 'TSLA' not in symbol_to_id:
    tsla_names = [k for k in symbol_to_id.keys() if 'TSLA' in k.upper()]
    if tsla_names:
        sym = tsla_names[0]
        print('Found as: %s' % sym)
    else:
...
print('LONG: %s (%d/13)' % (rating_l, long_score))
print('SHORT: %s (%d/13)' % (rating_s, short_score))
...
if long_score > short_score and long_score >= 7:
    ...
    if mn_trend == 'BULL':
        print('  Monthly BULL - wait for H4 RSI < 35 to go LONG')
    else:
        print('  Monthly BEAR - wait for H4 RSI > 65 to go SHORT')
